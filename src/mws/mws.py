...
class MWS(object):
	...
	def make_request(self, extra_data, method="POST", **kwargs):
		...
		try:
			...
			try:
				...
				parsed_response = xmltodict.parse(data)
			except:
	...
	def calc_signature(self, method, request_description):
		...
		data_bytes = bytes(sig_data, 'latin-1')  # Assumes `data` is also a string.
		hashed = hmac.new(key_bytes, data_bytes, hashlib.sha256).digest()
...
class Orders(MWS):
	...
	VERSION = "2013-09-01"
	NS = '{https://mws.amazonservices.in/Orders/2013-09-01}'
